...
class QLearning():
    def __init__(self, actions, learning_rate=0.05, reward_discount = 0.9, episode = 200,greedy=0.8):
    ...
    def update_q_table(self,state,action,reward,state_,done):
        #self.check_state_exist(state)
        ...
        #表示每次更新的变化量
        delta = abs(self.learning_rate * (new_value - old_value))
    ...
    def training(self):
        for i in range(self.episode):
            ...
            while True:
                step +=1
                action = self.choose_action(self.state,self.greedy)
                state_,reward,done = self.maze.step(self.state,action)
                #print(self.state,reward,state_)
                self.update_q_table(self.state,action,reward,state_,done)
                if(step>50):
                    break
                if done:
